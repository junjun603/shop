...
<script>
	...
	if(erros==1){
        toast('您还没有考场列表');
        setTimeout(function() {
            window.location.href = 'http://www.emsminghui.com/index.php?exam';
		},800)
    ...
    function toast(txt, fun) { //吐丝提示
        $('.toast').remove();
        var div = $('<div style="background:#5d5d5d;max-width: 85%;min-height: 77px;min-width: 270px;position: absolute;left: -1000px;top: -1000px;text-align: center;border-radius:10px;"><span style="color: #ffffff;line-height: 77px;font-size: 23px;">' + txt + '</span></div>');
        $('body').append(div);
