				...
				<div class="box itembox" style="padding-top:10px;margin-bottom:0px;overflow:visible">
					...
					<form action="index.php?certificate-master-certificate-add" method="post" class="form-horizontal">
				        ...
				        <div class="form-group">
				            ...
				            -->
							<label for="contenttitle" class="control-label col-sm-2">选择考场：</label>
...
<script>
	$(function(){
       // $('.courseList')
        $.ajax({
            type:"get",
            url:'index.php?certificate-master-certificate-courseList',
            success:function(res){
                var jsonObj = JSON.parse(res);
                var courStr = '<select class="selCou" name="args[cebasic]" >';
                for(var i=0;i<jsonObj.length;i++){
                    courStr += '<option value="'+jsonObj[i]['ceid']+'">'+jsonObj[i]['cetitle']+'</option>';
                }
                courStr += '</select>';
                $('.courseList').html(courStr);
            },
            error:function(){
                alert('网络错误');
            }
