...
class action extends app
{
	...
	private function batupdate(){
		$res = $this->db->batupdate();
		exit();
	}

	private function  getList(){ //获取角色或者 员工列表
        $flag = $_GET['flag_id'];
        $res = $this->db->selRoleOrEmp($flag);
					    ...
					    if($strings->isUserName($args['username']))
					    {
						    ...
						    if(!$u)
						    {
							   ...
							   // if($strings->isEmail($args['useremail']))
                                if($strings->isEmail($args['useremail']) || true)
								{
								    ...
								    if(!$u)
								    {
                                        ...
                                        $args['userpassword'] = $data[2];
								    	$args['usergroupid'] = $defaultgroup['groupid'];

                                        if(!$data[3]){
                                        	$data[3] = '阿凡提';
                                        }else{
                                            $data[3] = iconv('gb2312','utf-8',$data[3]);
										}
                                        $args['real_name'] = $data[3];

                                        if(!$data[4])$data[4] = '110';
                                        $args['userphone'] = $data[4];

                                        if(!$data[5]){
                                        	$data[5] = '男';
                                        }else{
                                            $data[5] = iconv('gb2312','utf-8',$data[5]);
										}
                                        $args['usergender'] = $data[5];

                                        if(!$data[6])$data[6] = '******';
                                        $args['userpassport'] = $data[6];
